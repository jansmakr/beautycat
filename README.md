...
- **현재 버전**: v2.8.13.6.36 (공지사항 레이아웃 최적화)
- **마지막 업데이트**: 2025-12-18
- **상태**: 🟢 프로덕션 운영 중
...
## 🚀 최근 업데이트 (v2.8.13.6.36)

### 2025-12-18 공지사항 통합 표시 + 레이아웃 최적화 (v2.8.13.6.36) 🎨
**"운영자 공지 + 샵 공지 통합 / 데스크탑 중앙 정렬 / 모바일 가로 배치" ✅**:

1. **문제 상황** 🚨:
   - 샵 공지가 표시되지 않음 (`shop_announcements` 테이블 미호출)
   - 운영자 공지만 표시 (`announcements` 테이블만 호출)
   - 데스크탑: 공지사항이 좌측에 몰려있어 시각적 불균형
   - 모바일: 텍스트가 세로로 배치되어 가독성 저하

2. **공지사항 통합 로직** ✅:
   - ✅ **두 테이블 통합**:
     * `tables/announcements` (운영자 공지)
     * `tables/shop_announcements` (샵 공지)
     * 두 테이블을 병합하여 함께 표시
...
3. **레이아웃 개선** ✅:
   - ✅ **데스크탑**:
     * 컨테이너: `max-w-4xl` → `max-w-3xl` (너비 최적화)
     * 정렬: `text-center` + `items-center` 추가 (완벽한 중앙 정렬)
     * 각 공지: `justify-center` + `min-width: 80%` (중앙 집중)
     * 간격: `gap: 6px` → `gap: 8px` (여유 공간)
     * 호버: `rounded-md` 추가 (시각적 피드백 개선)
   
   - ✅ **모바일**:
     * 컨테이너: `px-3` 패딩 추가 (좌우 여백 확보)
     * 각 공지: `width: 100%` 명시 (가로 펼침 보장)
     * 간격: `gap: 4px` → `gap: 6px` (가독성 개선)
     * 호버: `rounded-md` 추가 (터치 피드백)

3. **효과** 🎯:
   - ✅ 데스크탑: 공지사항이 화면 중앙에 균형있게 배치
   - ✅ 모바일: 4개 공지가 명확한 가로 줄로 표시
