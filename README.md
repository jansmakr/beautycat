...
#### 1. **📣 메인 페이지 우측 "샵 이벤트 및 공지" 사이드바**
- **관리자 공지 1개 + 샵 공지 3개** = 총 4개 표시 (제목만)
- **하늘색 그라데이션 배경**으로 강조 (#E0F2FE → #BAE6FD)
- 하늘색 테두리 (2px solid #38BDF8)
- 배지로 구분: "운영팀" (빨간색) / "샵" (파란색)
- 제목 한 줄로 깔끔하게 표시 (4줄 레이아웃)
- 클릭 시 상세 모달 팝업
- 조회수 자동 증가
- "모든 공지사항 보기 →" 버튼 (하늘색 테마)
- **모바일 최적화**: 모든 기기에서 표시 (반응형 크기 조정)
