...
function initializeApp() {
    ...
    // 대표샵 데이터 먼저 로드하고, 완료 후 시스템 초기화
    loadRepresentativeShops()
        .then(() => {
            console.log('✅ 대표샵 데이터 로드 완료 → 시스템 초기화');
            initializeRepresentativeShops();
        })
        .catch(() => {
            console.warn('⚠️ 대표샵 데이터 로드 실패 → 폴백 데이터로 초기화');
            initializeRepresentativeShops();
        });
    
    // 🔥 HOTFIX: 프로덕션에서도 사용자 자동입력 실행
    setupUserAutoFill();
...
function updateAuthUI() {
    ...
    if (currentUser) {
        // 로그인된 상태
        console.log('✅ 로그인 상태 - 대시보드 버튼 표시');
        if (loginBtn) loginBtn.classList.add('hidden');
        if (loggedInMenu) loggedInMenu.classList.remove('hidden');
    } else {
        // 로그인되지 않은 상태
        console.log('ℹ️ 비로그인 상태 - 로그인 버튼 표시');
...
async function loadRepresentativeShops() {
    try {
        ...
        // API에서 빈 데이터를 받았다면 폴백 데이터 사용
        if (representativeShopsData.length === 0) {
            console.warn('⚠️ API에서 빈 데이터 수신 → 폴백 데이터 사용');
