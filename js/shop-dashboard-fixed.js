...
async function loadConsultationRequests() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/tables/consultations?limit=100&sort=created_at`, {
            signal: controller.signal
        });
        clearTimeout(timeoutId);
        const data = await response.json();
...
async function loadQuotes() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/tables/quotes?limit=100&sort=created_at`, {
            signal: controller.signal
        });
        clearTimeout(timeoutId);
        const data = await response.json();
...
async function loadAnnouncementAlert() {
    try {
        const response = await fetch(`${API_BASE_URL}/tables/announcements?limit=1&sort=created_at`);
        const data = await response.json();
